// Resend Email Integration Helper for Pretty Village Musanze
// Configured to send notifications for inquiries and bookings to both:
...
export const INT_PHONE_1 = "+250792500176";
export const INT_PHONE_2 = "+250790156224";

export const WHATSAPP_LINK_1 = "https://wa.me/250792500176";
export const WHATSAPP_LINK_2 = "https://wa.me/250790156224";
...
/**
 * Sends email via Resend REST API to management recipients.
 */
async function sendResendRequest(subject: string, htmlContent: string, textContent: string) {
  const apiKey = (import.meta as any).env?.VITE_RESEND_API_KEY || (typeof process !== "undefined" ? process.env?.RESEND_API_KEY : "");

  if (!apiKey) {
    console.warn(
      "Resend API key missing (VITE_RESEND_API_KEY). Email notification logged locally for management recipients:",
      RECIPIENT_EMAILS,
      { subject, textContent }
    );
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "Pretty Village Reservations <onboarding@resend.dev>",
        to: RECIPIENT_EMAILS,
        subject,
        html: htmlContent,
        text: textContent,
    ...
    if (!res.ok) {
      ...
      console.warn("Resend API response error:", res.status, errText);
...
Send direct reply to: ${inquiry.email}
  `.trim();
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f9fbf9; padding: 24px; border-radius: 12px; border: 1px solid #e2e8f0;">
      ...
      <div style="margin-top: 24px; padding-top: 16px; border-top: 1px solid #e2e8f0; font-size: 12px; color: #a0aec0; text-align: center;">
        Pretty Village Musanze · Receiving Emails: prettyvillagee@gmail.com & feliciencalylx@gmail.com
      </div>
    </div>
  `;

  return sendResendRequest(subject, htmlContent, textContent);
}
