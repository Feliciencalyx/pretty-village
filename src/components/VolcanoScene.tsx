  ...
  return (
    <group ref={g}>
      ...
      <mesh position={[0, -0.4, 0]}>
        ...
        <meshStandardMaterial color="#a0722e" roughness={0.9} flatShading />
      </mesh>
      {/* Second peak */}
      <mesh position={[-1.6, -0.7, -0.5]} scale={0.75}>
        <coneGeometry args={[1.4, 2, 6, 1]} />
        <meshStandardMaterial color="#6b3a1c" roughness={0.9} flatShading />
      </mesh>
      {/* Third peak */}
      <mesh position={[1.5, -0.85, -0.4]} scale={0.6}>
        <coneGeometry args={[1.3, 1.8, 6, 1]} />
        <meshStandardMaterial color="#c9902d" roughness={0.9} flatShading />
      </mesh>
      {/* Golden cap on main */}
      <mesh position={[0, 0.75, 0]}>
        <coneGeometry args={[0.5, 0.6, 6, 1]} />
        <meshStandardMaterial color="#f0d78c" roughness={0.4} flatShading emissive="#c9a84c" emissiveIntensity={0.2} />
      </mesh>
      {/* Warm ring */}
      <mesh position={[0, -0.2, 0]} rotation={[Math.PI / 2, 0, 0]}>
        <torusGeometry args={[2.2, 0.15, 12, 40]} />
        <meshStandardMaterial color="#f5c76a" transparent opacity={0.3} />
      </mesh>
    ...
    <div className="h-[500px] w-full md:h-[600px]">
      <Canvas camera={{ position: [4, 2, 5], fov: 45 }} dpr={[1, 2]}>
        <Suspense fallback={null}>
          ...
          <directionalLight position={[5, 8, 5]} intensity={1.3} color="#ffdca0" />
          <directionalLight position={[-5, 3, -5]} intensity={0.4} color="#f5c76a" />
          <fog attach="fog" args={["#f0d78c", 8, 18]} />
